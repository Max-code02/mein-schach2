...
const CONFIG = {
    TABLES_TO_BACKUP: ['players', 'messages'],
...
async function runBackup(db) {
    ...
    try {
        if (!db) {
            console.error("❌ Firestore DB instance not provided to runBackup");
            return;
        }
        ...
        for (const tableName of CONFIG.TABLES_TO_BACKUP) {
            console.log(`⏳ [DB] Lade Collection: ${tableName}...`);
            let data = [];
            try {
            ...
            if (data.length < CONFIG.MIN_ROWS_EXPECTED && tableName === 'players') {
                throw new Error(`KRITISCHER FEHLER: Collection ${tableName} ist fast leer! Backup abgebrochen.`);
            }
...
function cleanOldBackups(directory) {
    try {
        ...
        let deletedCount = 0;
        files.forEach(file => {
            if (file === CONFIG.LOG_FILE) return;
