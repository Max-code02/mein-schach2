...
const addChatOld = `    if (type === "system") {
...
const addChatNew = `    if (type === "system") {
        m.textContent = "⚙️ " + text;
    } else {
        const strong = document.createElement("strong");
        strong.textContent = sender + ": ";
...
if (code.includes(addChatOld)) {
    code = code.replace(addChatOld, addChatNew);
    code = regexLogic + "\n" + code;
    fs.writeFileSync('script.js', code);
    console.log("Patched script.js for Perl RegEx interceptor");
} else {
    console.log("Could not find addChat code block");
}
